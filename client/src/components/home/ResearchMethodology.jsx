  ...
  return (
    <section className="py-12 md:py-20 bg-white text-center">
      <div>
        ...
        <h2 className="text-3xl md:text-5xl font-black text-black mb-4">Research Methodology</h2>
          ...
          <StaggerContainer className="flex flex-col md:flex-row justify-between relative gap-12 md:gap-0">
            ...
            {steps.map((step, index) => (
              <StaggerItem key={index} className="flex-1 flex flex-col items-center relative md:pt-6">
                ...
                {/* Text */}
                <h3 className="font-bold text-2xl md:text-xl text-black mt-8 md:mt-4 whitespace-pre-line leading-snug max-w-[250px] md:max-w-[150px]">
                  {step.replace(' & ', ' &\n')}
                </h3>
              </StaggerItem>
            ))}
